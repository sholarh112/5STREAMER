...
console.log(album_container.innerHTML);
...
xhr.addEventListener('readystatechange', function () {
	if (this.readyState === this.DONE) {
		...
		for (i=0; i<api.tracks.data.length; i++){
		  ...
		  <li
		  class="bg-[#09090941] mb-5 py-5 grid grid-cols-6 place-items-center"
		  >
		  <span>${track.data[i].position}</span>
				...
				<button class="ml-28 cursor-pointer" id="play_btn">play</button>
				<span class="ml-28 cursor-pointer">like</span>
			  </li>
		  `
		}
	}
